...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < M; i++) {
			...
			sb.append(queryMin(1, 0, N - 1, queryLeft, queryRight)).append(" ")
					.append(queryMax(1, 0, N - 1, queryLeft, queryRight)).append("\n");
		}
	...
	private static long queryMax(int node, int nodeLeft, int nodeRight, int queryLeft, int queryRight) {
		...
		long leftMax = queryMax(node << 1, nodeLeft, mid, queryLeft, queryRight);
		long rightMax = queryMax((node << 1) + 1, mid + 1, nodeRight, queryLeft, queryRight);
